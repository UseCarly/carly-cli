...
const description = {
  displayName: 'Carly',
  name: 'carly',
  // Themed SVG variants — n8n's lint wants a vector icon, and the { light,
  // dark } form so it reads on both themes. Assets come from the brand set in
  // use_carly_landing/public/brand.
  icon: { light: 'file:carly.svg', dark: 'file:carly.dark.svg' },
  group: ['transform'],
  version: 1,
  ...
  usableAsTool: true,
  // Placeholders swapped for the NodeConnectionTypes.Main enum after
  // serialization — n8n's lint rejects the bare "main" string literal.
  inputs: ['__MAIN__'],
  outputs: ['__MAIN__'],
...
function serialize(value: unknown): string {
  return JSON.stringify(value, null, 2)
    ...
    .replace(/"__MAIN__"/g, 'NodeConnectionTypes.Main');
}

const body = `import { NodeConnectionTypes } from 'n8n-workflow';
